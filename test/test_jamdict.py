...
from jamdict import Jamdict, JMDictXML, config
from jamdict.jmdict import JMDEntry, JMDictXMLParser
from jamdict.kanjidic2 import Kanjidic2XMLParser
from jamdict.old.util_old import _JAMDICT_DATA_AVAILABLE
...
class TestConfig(unittest.TestCase):
    def test_config(self):
        ...
        getLogger().info(
            "jamdict log file location: {}".format(
...
class TestModels(unittest.TestCase):
    def test_basic_models(self):
        ...
        self.assertEqual(
            str(e[0]), "repetition mark in katakana ((noun (common) (futsuumeishi)))"
        )
        self.assertEqual(
            str(e[0].text()), "repetition mark in katakana"
        )  # compact is enabled by default
...
class TestJamdictXML(unittest.TestCase):
    ...
    def test_jmdict_xml(self):
        ...
        self.assertTrue(jmd.lookup("おてんき"))

    def test_jmdict_fields(self):
        parser = JMDictXMLParser()
        entries = parser.parse_file(MINI_JMD)
        jmd = JMDictXML(entries)
        results = jmd.lookup("おてんき")
...
class TestJamdictSQLite(unittest.TestCase):
        ...
        with self.assertLogs(level="WARNING") as cm:
            ...
            for line in cm.output:
                if "POS filter should be a collection, not a string" in line:
                    ...
                    break
                if "pos filter should be a list, not a string" in line:
                    warned_pos_as_str = True
                    break
    ...
    def test_search_by_ne_type(self):
        ...
        self.assertEqual(expected, set(netypes))
    ...
    @unittest.skipIf(
        not _JAMDICT_DATA_AVAILABLE,
        "Jamdict data is not available. Data test is skipped",
    )
    def test_jamdict_data(self):
        jam = Jamdict()
        # search verb kaeru
        res = jam.lookup("かえる", pos="transitive verb")
        actual = [int(e.idseq) for e in res.entries]
        ...
        actual2 = [int(e.idseq) for e in res2.entries]
