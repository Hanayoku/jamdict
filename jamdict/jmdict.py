...
class JMDEntry(object):
    ...
    def __init__(self, idseq=""):
        ...
        self.info: EntryInfo | None = None  # info?   => EntryInfo
        ...
        if self.senses:
            ...
            if len(self.senses) == 1:
                ...
                for sense, idx in zip(self.senses, range(len(self.senses))):
                    tmp.append(
                        "{i}. {s}".format(i=idx + 1, s=sense.text(compact=compact))
                    )
...
class KanaForm(object):
    ...
    def to_dict(self):
        knd = {"text": self.text, "nokanji": self.nokanji}
        if self.restr:
...
class Link(object):
    ...
    def to_dict(self):
        return {"tag": self.tag, "desc": self.desc, "uri": self.uri}
...
class Sense(object):
    ...
    def text(self, compact=True):
        ...
        if not compact and self.pos:
            return "{gloss} ({pos})".format(
                gloss="/".join(tmp), pos=("(%s)" % "|".join(self.pos))
            )
        else:
            return "/".join(tmp)
...
class Translation(Sense):
    ...
    def name_type_human(self):
        return [
            JMENDICT_TYPE_MAP[x] if x in JMENDICT_TYPE_MAP else x
            for x in self.name_type
        ]

    def text(self, compact=True):
        tmp = [str(x) for x in self.gloss]
        types = (
            "/".join(self.name_type) if compact else "/".join(self.name_type_human())
        )
...
class JMDictXMLParser(object):
    ...
    def get_single(self, tag_name, a_tag):
        ...
        if len(children) == 0:
            ...
            raise Exception(
                "There are %s %s tags in %s" % (len(children), tag_name, a_tag.tag)
            )
        else:
            return children[0]
    ...
    def parse_info(self, info_tag, entry):
        ...
        for child in info_tag:
            if child.tag == "links":
                ...
                raise Exception(
                    "WARNING: invalid tag in info tag (child.tag = %s)" % child.tag
                )
        entry.set_info(einfo)
        return einfo

    def parse_link(self, link_tag, entry_info):
        tag_elem = self.get_single("link_tag", link_tag)
        desc_elem = self.get_single("link_desc", link_tag)
        uri_elem = self.get_single("link_uri", link_tag)
        if tag_elem is None or desc_elem is None or uri_elem is None:
            raise Exception(
                "Malformed <links> element: missing required child (link_tag, link_desc, or link_uri)"
            )
        link = Link(tag_elem.text, desc_elem.text, uri_elem.text)
        entry_info.links.append(link)
        return link

    def parse_audit(self, audit_tag, entry_info):
        upd_date_elem = self.get_single("upd_date", audit_tag)
        upd_detl_elem = self.get_single("upd_detl", audit_tag)
        if upd_date_elem is None or upd_detl_elem is None:
            raise Exception(
                "Malformed <audit> element: missing required child (upd_date or upd_detl)"
            )
        audit = Audit(upd_date_elem.text, upd_detl_elem.text)
        entry_info.audit.append(audit)
        return audit
    ...
    def parse_bibinfo(self, bib_tag, entry_info):
        ...
        for child in bib_tag:
            if child.tag == "bib_tag":
                ...
                raise Exception(
                    "WARNING: invalid tag in bibinfo (child.tag = %s)" % child.tag
                )
    ...
    def parse_ne_translation(self, trans_tag, entry):
        ...
        for child in trans_tag:
            if child.tag == "name_type":
                _name_type = (
                    JMENDICT_TYPE_MAP_DECODE[child.text]
                    if child.text in JMENDICT_TYPE_MAP_DECODE
                    else child.text
                )
                ...
                raise Exception(
                    "Invalid tag: {} in JMendict/trans tag".format(child.tag)
                )
    ...
    def parse_sense(self, sense_tag, entry):
        ...
        for child in sense_tag:
            if child.tag == "stagk":
                ...
                raise Exception(
                    "WARNING: invalid tag in sense tag (child.tag = %s) content = %s"
                    % (child.tag, etree.tostring(child))
                )
