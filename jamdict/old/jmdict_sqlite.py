...
INSERT INTO meta VALUES ('generator_url', '{gu}');""".format(
    jv=JMDICT_VERSION, ju=JMDICT_URL, gv=JAMDICT_VERSION, gu=JAMDICT_URL
)
...
class JMDictSQLite(JMDictSchema):
    ...
    def _build_search_query(self, query, pos=None):
        ...
        if query.startswith("id#"):
            ...
            if _is_wildcard_search:
                where.append(
                    "(idseq IN (SELECT idseq FROM Kanji WHERE text like ?) OR idseq IN (SELECT idseq FROM Kana WHERE text like ?) OR idseq IN (SELECT idseq FROM sense JOIN sensegloss ON sense.ID == sensegloss.sid WHERE text like ?))"
                )
            else:
                where.append(
                    "(idseq IN (SELECT idseq FROM Kanji WHERE text == ?) OR idseq IN (SELECT idseq FROM Kana WHERE text == ?) OR idseq IN (SELECT idseq FROM sense JOIN sensegloss ON sense.ID == sensegloss.sid WHERE text == ?))"
                )
        ...
        if pos:
            ...
            if where:
                where.append("AND")
            where.append(
                f"idseq IN (SELECT idseq FROM Sense WHERE ID IN (SELECT sid FROM pos WHERE text IN ({','.join('?' * slots)})))"
            )
            params += pos
        # else (a context is provided)
    ...
    def search_iter(self, query, ctx=None, pos=None, **kwargs):
        ...
        if ctx is None:
            with self.ctx() as ctx:
                return self.search_iter(query, ctx=ctx, pos=pos)
        where, params = self._build_search_query(query, pos=pos)
        where.insert(0, "SELECT idseq FROM Entry WHERE ")
        for (idseq,) in ctx.conn.cursor().execute(" ".join(where), params):
